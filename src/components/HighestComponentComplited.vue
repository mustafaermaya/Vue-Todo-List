<template>
  <div class="container f">
    <div v-if="!todoslistHighest.length || !todoprioritylistHighest.length">
      <h3 class="highp">Highest Complited</h3>
      <p>Nothing on the complited highest ToDo list...</p>
    </div>
    <div v-else class="container">
      <h3 class="p">Complited Highest Todo List</h3>
      <ul>
...
export default {
  ...
  created() {
    eventBus.$on("removedTodolistHighest", todocomplited => {
      this.todoslistHighest.push(todocomplited);
    });
    eventBus.$on("removedTodoListHighestPriority", prioritycomplited => {
